...
namespace Craveva\Craveva\Functions;

use Craveva\Craveva\Helpers\CravevaApp;
...
class CravevaUpdate
{
    ...
    {
        $setting = config('craveva.setting');
    ...
    {
        $setting = config('craveva.setting');
        $cravevaUpdateCompanySetting = (new $setting)::first();

        // Check conditions for showing the review modal
        return (
            !is_null($cravevaUpdateCompanySetting->supported_until) &&
            !$cravevaUpdateCompanySetting->supportedUntilIsPast() &&
            (
                $cravevaUpdateCompanySetting->isWithin175Days() ||
                $cravevaUpdateCompanySetting->isBeyond200DaysAndWithin360Days()
            ) &&
            $cravevaUpdateCompanySetting->show_review_modal === 1
        );
    }
    ...
    public static function reviewUrl()
    {
        ...
        $url = str_replace('verify-purchase', 'review', config('craveva.verify_url'));

        return $url . '/' . $cravevaUpdateCompanySetting->purchase_code;

    }

    public static function plugins()
    {
        return CravevaApp::getRemoteData(config('craveva.plugins_url'));
    }
    ...
    public static function updateVersionInfo()
    {
        ...
        try {
            ...
            $lastVersion = CravevaApp::getRemoteData(config('craveva.updater_file_path'));
        ...
        try {
            ...
            $lastVersionLog = CravevaApp::getRemoteData(config('craveva.versionLog') . '/' . File::get('version.txt'));
    ...
    public static function curl($postData)
    {
       ...
       try {
           $ch = curl_init();
           curl_setopt($ch, CURLOPT_URL, config('craveva.verify_url'));
