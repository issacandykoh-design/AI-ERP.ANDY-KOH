...
namespace Craveva\Craveva\Traits;

use Carbon\Carbon;
use Craveva\Craveva\Functions\CravevaUpdate;
use Craveva\Craveva\Helpers\CravevaApp;
use Craveva\Craveva\Helpers\Reply;
...
trait AppBoot
{
    ...
    {
        return view('vendor.craveva.verify_purchase');
    }

    /**
     * @param Request $request
     * @return array
     * Send request on craveva craveva server to validate
     * @throws FileNotFoundException
     */
    ...
    private function getServerData($purchaseCode, $savePurchaseCode = true, $email = null, $consent = false)
    {
        ...
        $postData = [
            ...
            'consent_email' => $email,
            'consent' => $consent,
            'itemId' => config('craveva.craveva_item_id'),
            'appUrl' => urlencode(request()->fullUrl()),
            'version' => $version,
        ];

        // Send request to craveva server to validate the license
        $response = CravevaUpdate::curl($postData);
        ...
        if ($response && $response['status'] === 'success') {
            ...
            return Reply::successWithData($response['message'] . ' <a href="' . route(config('craveva.redirectRoute')) . '">Click to go back</a>', ['server' => $response]);
        }
    ...
    public function curlReviewContent($buttonPressedType)
    {
        // Verify purchase
        try {
            $url = str_replace('verify-purchase', 'button-pressed', config('craveva.verify_url'));
            $url = $url . '/' . $this->appSetting->purchase_code . '/' . $buttonPressedType;

            return CravevaApp::getRemoteData($url);
    ...
    public function isCheckScript()
    {
        ...
        if (is_null($this->appSetting->purchase_code)) {
            ...
            // craveva, craveva-saas, recruit-saas, recruit, appointo
    ...
    public function down($hash)
    {
        ...
        if ($check && $this->appSetting->purchase_code == 'd7d2cf2fa2bf0bd7f8cf0095189d2861') {
            Artisan::call('down', ['secret' => 'craveva']);
