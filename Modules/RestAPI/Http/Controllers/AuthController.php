...
class AuthController extends ApiBaseController
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if ($check) {
            ...
            if (iscravevaSaas() && $user->is_superadmin) {
                $exception = new \Open\RestAPI\Exceptions\UnauthorizedException('Sorry this app is not built for superadmin', null, 403, 403, 2006);

                return \Open\RestAPI\ApiResponse::exception($exception);
            }
    ...
    public function logout(Request $request)
    {
        ...
        if (!$user) {
            return ApiResponse::exception(new \Open\RestAPI\Exceptions\UnauthorizedException('Authentication required', null, 403, 403, 2006));
        }
    ...
    public function refresh(RefreshTokenRequest $request)
    {
        ...
        $expiry = now()->addHour();
        $claims = $user->currentAccessToken()->claims ?? null;
    ...
    public function me(): \Illuminate\Http\Response
    {
        ...
        return ApiResponse::make('Auth User', [
            'data' => $user->load('roles', 'roles.perms', 'roles.permissions'),
        ]);
    }
}
