...
class CustomModuleController extends AccountBaseController
{
    ...
    {
        // Module verification removed - redirect to module list
    ...
    {
        $cravevaPath = storage_path('app') . '/Modules/craveva';
        $zip->extract($cravevaPath);
        $files = File::allfiles($cravevaPath);

        foreach ($files as $file) {

            if (str_contains($file->getRelativePathname(), '.zip')) {
                $filePath = $file->getRelativePathname();
                $zip = Zip::open($cravevaPath . '/' . $filePath);
                $zip->extract(storage_path('app') . '/Modules');
    ...
    private function updateVersion($moduleName)
    {
        try {
            ...
            // Module verification removed - version update continues without verification
