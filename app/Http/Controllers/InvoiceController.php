...
class InvoiceController extends AccountBaseController
{
    ...
    public function ocrScan(Request $request)
    {
        try {
            // 验证上传的文件
            $request->validate([
                'image' => 'required|image|mimes:jpeg,png,jpg,gif|max:10240', // 最大10MB
            ]);
            ...
            // 创建临时文件路径
            $tempPath = storage_path('app/temp/ocr_' . time() . '_' . $image->getClientOriginalName());
            ...
            // 使用OCR服务进行文字识别
            $ocrService = app(\Modules\OCR\Services\OCRService::class);
            $extractedText = $ocrService->extractTextFromPath($tempPath);
