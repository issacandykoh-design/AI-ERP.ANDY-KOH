...
class FrontendController extends FrontBaseController
{
    ...
    public function index($slug = null)
    {
        ...
        if ($this->global->setup_homepage == 'custom') {
            // Redirect to the external/custom homepage so all assets resolve against its origin
            return redirect()->away($this->global->custom_homepage_url);
        }
        ...
        try {
            ...
            $counts = Feature::select(
                    DB::raw('count(case when type = "image" then 1 end) as imageFeaturesCount'),
                    DB::raw('count(case when type = "icon" then 1 end) as iconFeaturesCount')
                )
                ->where('language_setting_id', $localeLanguageId)
                ->groupBy('language_setting_id')
                ->first();

            $imageFeaturesCount = $counts ? ($counts->imageFeaturesCount ?? 0) : 0;
            $iconFeaturesCount = $counts ? ($counts->iconFeaturesCount ?? 0) : 0;
            ...
            \Log::error('home_build_failed: '.$e->getMessage());
            $this->seoDetail = null;
            $this->pageTitle = __('app.menu.home');
            $this->currencies = collect();
            $this->packages = collect();
            $this->featureWithImages = collect();
            $this->featureWithIcons = collect();
            $this->frontClients = collect();
            $this->testimonials = collect();
            $this->trialPackage = null;
            $this->packageSetting = null;
        }

        // Theme 3: Modern React-based landing page (if front_design == 2)
        if ($this->global->front_design == 2) {
            $ua = request()->header('User-Agent', '');
            $isMobile = preg_match('/Mobile|Android|iPhone|iPad|iPod/i', $ua);
            $candidates = $isMobile
                ? [
                    public_path('public/landing/mobile.html'),
                    public_path('landing/mobile.html'),
                    public_path('public/landing/index.html'),
                    public_path('landing/index.html')
                  ]
                : [
                    public_path('public/landing/web.html'),
                    public_path('landing/web.html'),
                    public_path('public/landing/index.html'),
                    public_path('landing/index.html')
                  ];
            $landing = null;
            foreach ($candidates as $path) {
                if (file_exists($path)) { $landing = $path; break; }
            }
            if ($landing) {
                $content = file_get_contents($landing);
                
                // Inject theme settings into the page
                $frontDetail = FrontDetail::first();
                $primaryColor = $frontDetail ? ($frontDetail->primary_color ?? '#453130') : '#453130';
                $lightColor = $frontDetail && $frontDetail->light_color ? $frontDetail->light_color : ($primaryColor . '26');
                ...
                $content = str_replace('</head>', $themeCss . '</head>', $content);
                
                return response($content)->header('Content-Type', 'text/html');
            }
            
            // Fallback to Theme 2 if landing page doesn't exist
            if ($slug) {
        ...
        // Theme 2: Multi-Page design (front_design == 1)
        ...
        // Theme 1: Single page design is selected (front_design == 0)
