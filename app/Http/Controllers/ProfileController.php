...
$logout = false;
        ...
        if ($locale) {
            ...
            // Special-case Chinese locales to match folder names
            if (str_starts_with(strtolower($normalized), 'zh-')) {
                $parts = explode('-', $normalized, 2);
                $normalized = $parts[0] . '-' . strtoupper($parts[1] ?? '');
            }
            // Map non-standard code 'tw' to 'zh-TW'
            if (strtolower($normalized) === 'tw') {
                $normalized = 'zh-TW';
            }
            $locale = $normalized;
        }
        ...
        // Sync session locale to user preference to ensure immediate effect across authenticated panels
        if ($user->locale) {
        ...
        if ($user->clientDetails) {
            ...
            // cravevaSAAS move outside addEmployeeDetail for cravevasaas
