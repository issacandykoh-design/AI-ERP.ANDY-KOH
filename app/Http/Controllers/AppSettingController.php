...
class AppSettingController extends AccountBaseController
{
    ...
    public function update(UpdateAppSetting $request, $id)
    {
        ...
        switch ($tab) {
        case 'file-upload-setting':
            iscravevaSaas() ? $this->updateFileUploadSetting($request) : '';
            break;
        case 'client-signup-setting':
            $this->updateClientSignupSetting($request);
            break;
        case 'google-map-setting':
            iscravevaSaas() ? $this->updateGoogleMapSetting($request) : '';
            break;
        default:
    ...
    public function updateAppSetting($request)
    {
        ...
        // cravevaSAAS change language for front detail
        $frontDetail = FrontDetail::first();
        if ($frontDetail) {
            $frontDetail->locale = $request->locale;
            $frontDetail->save();
        }
