...
class EstimateController extends AccountBaseController
{
    ...
    public function addItem(Request $request)
    {
        ...
        $pricingData = null;
        $bundleId = $request->get('bundleId') ?? $request->get('bundle_id');
        if ($bundleId && DB::table('product_bundles')
            ->where('product_id', $this->items->id)
            ->where('id', $bundleId)
            ->where('is_active', 1)
            ->exists()) {
            $payload = [
                'option_item_ids' => (array) ($request->get('option_item_ids') ?? []),
                'items' => (array) ($request->get('items') ?? [])
            ];
            $resp = Http::post(url('api/products/' . $this->items->id . '/bundles/' . $bundleId . '/price'), $payload);
            if ($resp->successful()) {
                $pricingData = $resp->json();
                $this->items->price = number_format((float) ($pricingData['final_price'] ?? $this->items->price), 2, '.', '');
            }
        }
        ...
        return Reply::dataOnly(['status' => 'success', 'view' => $view, 'pricing_metadata' => $pricingData]);
    }
