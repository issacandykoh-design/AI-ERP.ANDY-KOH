...
class BaseNotification extends Notification implements ShouldQueue
{
    ...
    public function build($notifiable = null)
    {
        ...
        $defaultFromName = Config::get('mail.from.name') ?? Config::get('app.name');
        $defaultFromEmail = Config::get('mail.from.address') ?? 'no-reply@' . parse_url(Config::get('app.url'), PHP_URL_HOST);
        $replyName = $companyName = ($smtpSetting->mail_from_name ?? $defaultFromName);
        $configuredFromEmail = ($smtpSetting->mail_from_email ?? $defaultFromEmail);
        $smtpUsernameEmail = $smtpSetting->mail_username ?? $configuredFromEmail;
        $replyEmail = $configuredFromEmail;
        ...
        // If the application is craveva, return the mail message with SMTP settings
        if (iscraveva()) {
            return $build->from($configuredFromEmail, $companyName);
        }
        ...
        $fromEmail = (config('mail.verified') === true) ? $configuredFromEmail : $smtpUsernameEmail;
        \Log::info('MAIL_FROM_USED=' . $fromEmail);

        // Return the mail message with configured from and replyTo settings
        return $build->from($fromEmail, $replyName)->replyTo($replyEmail, $replyName);
    }
    ...
    public function resetLocale()
    {
        ...
        if (!is_null($company)) {
            ...
            App::setLocale($sessionLocale ?: $globalSetting->locale);
        }
    }
