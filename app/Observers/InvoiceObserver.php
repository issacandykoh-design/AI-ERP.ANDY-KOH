            ...
            if (!empty(request()->item_name) && is_array(request()->item_name)) {
                ...
                foreach (request()->item_name as $key => $item) :
                    ...
                    if (isset($invoiceItem) && (isset($invoice_item_image[$key]) || isset($invoice_item_image_url[$key]))) {
                        ...
                        $invoiceItemImage = InvoiceItemImage::create(
                            [
                        ...
                        // OCR处理：如果上传了图片文件，进行文字识别
                        if (isset($invoice_item_image[$key]) && $filename) {
                            try {
                                $ocrService = new OCRService();
                                $imagePath = public_path('user-uploads/' . InvoiceItemImage::FILE_PATH . '/' . $invoiceItem->id . '/' . $filename);
                                
                                // 检查文件是否存在
                                if (file_exists($imagePath)) {
                                    // 使用OCR服务提取文字，默认使用英文
                                    $extractedText = $ocrService->extractTextFromPath($imagePath, 'eng');
                                    
                                    // 如果提取到文字，更新发票项目的描述
                                    if (!empty($extractedText)) {
                                        $currentSummary = $invoiceItem->item_summary ?: '';
                                        $newSummary = $currentSummary ? $currentSummary . "\n\n[OCR识别文字]: " . $extractedText : "[OCR识别文字]: " . $extractedText;
                                        
                                        $invoiceItem->update(['item_summary' => $newSummary]);
                                    }
                                }
                            } catch (Exception $e) {
                                // OCR处理失败时记录日志，但不影响正常的发票创建流程
            ...
            if (!empty($request->item_name) && is_array($request->item_name) && !request()->has('cn_number')) {
                ...
                foreach ($items as $key => $item) {
                    ...
                    if ((isset($invoice_item_image[$key]) && $request->hasFile('invoice_item_image.' . $key)) || isset($invoice_item_image_url[$key])) {
                        ...
                        $invoiceItemImage = InvoiceItemImage::updateOrCreate(
                            [
                                'invoice_item_id' => $invoiceItem->id,
                            ],
                            [
