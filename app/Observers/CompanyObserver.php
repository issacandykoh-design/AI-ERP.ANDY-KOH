...
class CompanyObserver
{
    ...
    public function saving(Company $company)
    {
        ...
        if ($company->id === 1 && iscraveva() && ! isRunningInConsoleOrSeeding()) {
    ...
    public function updateModuleSettings($company): void
    {
        ...
        $existingModuleNames = $moduleSettings->pluck('module_name')->unique()->toArray();
        ...
        // Create ModuleSetting records for modules in package that don't exist yet
        // This handles cases where modules were added to package after company creation
        $modulesToCreate = $moduleInPackage->filter(function ($moduleName) use ($existingModuleNames) {
            return ! in_array($moduleName, $existingModuleNames);
        });

        if ($modulesToCreate->isNotEmpty()) {
            $data = [
                'admin' => [
                    ...ModuleSetting::CLIENT_MODULES, ...ModuleSetting::OTHER_MODULES,
                ],
            ...
            foreach ($data as $type => $moduleList) {
                foreach ($moduleList as $module) {
                    // Only create if module is in package and not already exists
                    if ($moduleInPackage->contains($module) && ! in_array($module, $existingModuleNames)) {
                        $newModuleSettings[] = [
                            'company_id' => $company->id,
                            'type' => $type,
                            'module_name' => $module,
            ...
            // Also check for custom modules (like biolinks, zoom, etc.) that might not be in CLIENT_MODULES or OTHER_MODULES
            foreach ($modulesToCreate as $moduleName) {
                // Check if this is a custom module that needs ModuleSetting records
                // Try to find if there's a module entity with addModuleSetting method
                $moduleClass = $this->findModuleSettingClass($moduleName);
                if ($moduleClass && method_exists($moduleClass, 'addModuleSetting')) {
                    // Let the module's addModuleSetting handle it (it will check package)
                    try {
                        $moduleClass::addModuleSetting($company);
                    } catch (\Exception $e) {
                        // If module doesn't have addModuleSetting, create basic records
                        foreach (['admin', 'employee'] as $type) {
                            if (! ModuleSetting::where('company_id', $company->id)
                    ...
                    // Create basic records for unknown modules
                    foreach (['admin', 'employee'] as $type) {
                        if (! ModuleSetting::where('company_id', $company->id)
                            ->where('module_name', $moduleName)
                            ->where('type', $type)
            ...
            if (! empty($newModuleSettings)) {
                ModuleSetting::insert($newModuleSettings);
            }
        }
    ...
    /**
     * Try to find the module's setting class
     */
    private function findModuleSettingClass(string $moduleName): ?string
    {
        $moduleNamePascal = ucfirst(str_replace(' ', '', ucwords(str_replace(['-', '_'], ' ', $moduleName))));

        $possibleClasses = [
            "Modules\\{$moduleNamePascal}\\Entities\\{$moduleNamePascal}GlobalSetting",
            "Modules\\{$moduleNamePascal}\\Entities\\{$moduleNamePascal}Setting",
            "Modules\\Craveva\\Modules\\{$moduleNamePascal}\\Entities\\{$moduleNamePascal}GlobalSetting",
            "Modules\\Craveva\\Modules\\{$moduleNamePascal}\\Entities\\{$moduleNamePascal}Setting",
        ];
