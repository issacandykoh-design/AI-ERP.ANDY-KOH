...
class SmtpSetting extends BaseModel
{
    ...
    public function verifySmtp()
    {
        ...
        try {
            ...
            // Authentication is handled automatically by Symfony Mailer when username/password are set
            // setAuthMode() method doesn't exist in Symfony Mailer - removed

            // Set local domain to prevent relay errors (if method exists)
            if (method_exists($transport, 'setLocalDomain')) {
                $transport->setLocalDomain($this->mail_host);
            }
