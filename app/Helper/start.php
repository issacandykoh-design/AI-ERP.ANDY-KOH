    ...
    function global_setting()
    {
        if (app()->environment('local')) {
            if (! cache()->has('global_setting')) {
                $setting = new \App\Models\GlobalSetting();
                $setting->locale = 'en';
                $setting->logo_background_color = '#ffffff';
                $setting->frontend_disable = 0;
                $setting->setup_homepage = 'login';
                $setting->custom_homepage_url = null;
                $setting->app_debug = true;
                $setting->front_design = 1;
    ...
    function language_setting()
    {
        if (app()->environment('local')) {
            if (! cache()->has('language_setting')) {
                cache(['language_setting' => collect([])]);
            }
...
if (! function_exists('user_modules')) {
    ...
    function user_modules()
    {
        ...
        foreach ($module->toArray() as $item) {
            $moduleName = array_values($item)[0];

            // Check if module is enabled in ModuleSwitch
            $moduleSwitch = \App\Models\ModuleSwitch::where('module_name', $moduleName)->first();

            // Only include module if it's enabled in ModuleSwitch (or if no ModuleSwitch record exists for backward compatibility)
...
function gdpr_setting()
{
        if (app()->environment('local')) {
            if (! session()->has('gdpr_setting')) {
                $setting = new GdprSetting();
                $setting->enable_gdpr = 0;
                session(['gdpr_setting' => $setting]);
            }
...
function social_auth_setting()
{
        if (app()->environment('local')) {
            if (! cache()->has('social_auth_setting')) {
                $socialAuth = new \stdClass();
                $socialAuth->google_status = 'disable';
                $socialAuth->facebook_status = 'disable';
                $socialAuth->twitter_status = 'disable';
                $socialAuth->linkedin_status = 'disable';
                $socialAuth->social_auth_enable = false;
                $socialAuth->social_auth_enable_count = 0;
                cache(['social_auth_setting' => $socialAuth]);
            }
...
function check_migrate_status()
{
        ...
        if (! session()->has('check_migrate_status')) {
            try {
                ...
                \Log::error('Migration failed during check_migrate_status: '.$e->getMessage());
                session(['check_migrate_status' => 'Error']);
            }
        }
...
if (! function_exists('module_enabled')) {
    ...
    {
        // First check if module exists in the filesystem
        $moduleExists = \Nwidart\Modules\Facades\Module::collections()->has($moduleName);

        if (! $moduleExists) {
            return false;
        }

        // Then check if module is enabled in ModuleSwitch
        $moduleSwitch = \App\Models\ModuleSwitch::where('module_name', $moduleName)->first();

        // Return true only if module exists and is enabled (or no ModuleSwitch record exists for backward compatibility)
        return ! $moduleSwitch || $moduleSwitch->is_enabled;
    }
}
    ...
    function currency_format_setting($currencyId = null)
    {
        if (! session()->has('currency_format_setting'.$currencyId)) {
            if ($currencyId == null) {
                ...
                // If currency with given ID not found, fallback to first currency
                if (! $setting) {
                    $setting = Currency::first();
                }
            }
...
if (! function_exists('currency_format')) {
    ...
    function currency_format($amount, $currencyId = null, $showSymbol = true)
    {
        ...
        // Check if $formats is null and provide default values
        if (is_null($formats)) {
            ...
            if (! $showSymbol) {
                ...
                $settings = $formats->company ?? ($formats && $formats->company_id ? Company::find($formats->company_id) : null);
                $currency_symbol = $currencyId == null ? ($settings && $settings->currency ? $settings->currency->currency_symbol ?? '' : '') : ($formats->currency_symbol ?? '');
            }
...
if (! function_exists('sidebar_superadmin_perms')) {
    ...
    function sidebar_superadmin_perms()
    {
        ...
        if (! session()->has('sidebar_superadmin_perms')) {
            ...
            foreach ($filteredPermissions as $item) {
                // Default superadmin permissions to "all" instead of "none"
                $sidebarSuperadminPermissions[$item->name] = 4;
            }
...
if (! function_exists('iscravevaSaas')) {

    function iscravevaSaas()
    {
        return strtolower(config('app.app_name')) === 'craveva-saas';
    }
}
...
if (! function_exists('iscraveva')) {
    ...
    function iscraveva()
    {
        return strtolower(config('app.app_name')) === 'craveva';
    }
}
...
if (! function_exists('getDomain')) {
    ...
    {
        if (! $host) {
            $host = $_SERVER['SERVER_NAME'] ?? 'craveva-saas.test';
        }
...
if (! function_exists('get_custom_menu_name')) {
    /**
     * Get custom menu name for a menu key, or return default
     */
    function get_custom_menu_name(string $menuKey, string $defaultName): string
    {
        ...
        try {
            ...
            return \App\Models\CustomMenuSetting::getCustomName(user()->company_id, $menuKey, $defaultName, $locale);
        } catch (\Exception $e) {
            return $defaultName;
        }
    }
...
if (! function_exists('get_custom_menu_order')) {
    /**
     * Get menu order for a menu key
     */
    function get_custom_menu_order(string $menuKey): int
    {
        if (! user() || ! user()->company_id) {
            return 999;
        }
        ...
        try {
            return \App\Models\CustomMenuSetting::getMenuOrder(user()->company_id, $menuKey);
        } catch (\Exception $e) {
            return 999;
        }
    }
...
if (! function_exists('is_menu_visible')) {
    /**
     * Check if menu is visible
     */
    function is_menu_visible(string $menuKey): bool
    {
        if (! user() || ! user()->company_id) {
            return true;
        }
        ...
        try {
            return \App\Models\CustomMenuSetting::isMenuVisible(user()->company_id, $menuKey);
        } catch (\Exception $e) {
            return true;
        }
    }
...
function isRtl($class = null)
{
        if (! session()->has('isRtl')) {
            ...
            if (app()->environment('local')) {
                session(['isRtl' => $rtl]);
                return is_null($class) ? $rtl : ($rtl ? $class : false);
            }
