                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="form-group cursor-pointer">
                            ...
                            <select name="theme" class="image-picker image-picker-theme show-labels show-html" id="theme-selector">
                                ...
                                <option data-img-src="{{ asset('img/new-design.jpg') }}" data-toggle="tooltip"
                                        data-original-title="Modern React Landing Page" @if ($global->front_design == 2) selected @endif
                                        value="2">Theme 3 (Modern Landing Page)
                                </option>
                            </select>
                            <div class="f-12 text-muted mt-2">
                                <strong>Current Selection:</strong> 
                                <span id="current-theme-display">
                                    @if($global->front_design == 0)
                                        Theme 1 (Classic Design)
                                    @elseif($global->front_design == 1)
                                        Theme 2 (Multi-Page Design)
                                    @elseif($global->front_design == 2)
                                        Theme 3 (Modern Landing Page) ✅
                                    @endif
                                </span>
                            </div>
                            <div class="mt-3" id="theme3-previews" @if($global->front_design != 2) style="display:none" @endif>
                                <a href="/public/landing/web.html" target="_blank" class="btn btn-primary btn-sm me-2">Preview Desktop</a>
                                <a href="/public/landing/mobile.html" target="_blank" class="btn btn-secondary btn-sm me-2">Preview Mobile</a>
                                <a href="/public/landing/index.html" target="_blank" class="btn btn-outline-primary btn-sm">Preview SPA</a>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Selecting the main theme
            ...
            changed: function (vale, newval) {
                console.log('Theme changed to:', newval[0]);
                var selectedTheme = newval[0];
                
                // Update current theme display
                updateThemeDisplay(selectedTheme);
                
                showLoginBlock(selectedTheme == '1' || selectedTheme == '2');
                showPrimaryColor(selectedTheme == '1' || selectedTheme == '2');
                if (selectedTheme == '2') { $('#theme3-previews').show(100); } else { $('#theme3-previews').hide(100); }
                
                // Show homepage settings for Theme 2 and Theme 3
                if (selectedTheme == '1' || selectedTheme == '2') {
                    $('#set-homepage-div, #set-homepage-banner-bg').show(100);
                } else {
                    $('#set-homepage-div, #set-homepage-banner-bg').hide(100);
                }
            },
            initialized: function (val) {
                var currentTheme = $(".image-picker-theme").val();
                console.log('Theme initialized with:', currentTheme);
                
                // Update current theme display
                updateThemeDisplay(currentTheme);
                
                showLoginBlock(currentTheme == '1' || currentTheme == '2');
                showPrimaryColor(currentTheme == '1' || currentTheme == '2');
                if (currentTheme == '2') { $('#theme3-previews').show(); } else { $('#theme3-previews').hide(); }
                
                // Show homepage settings for Theme 2 and Theme 3
                if (currentTheme == '1' || currentTheme == '2') {
                    $('#set-homepage-div, #set-homepage-banner-bg').show();
                } else {
                    $('#set-homepage-div, #set-homepage-banner-bg').hide();
                }
            }
        });
        
        function updateThemeDisplay(themeValue) {
            var themeNames = {
                '0': 'Theme 1 (Classic Design)',
                '1': 'Theme 2 (Multi-Page Design)',
                '2': 'Theme 3 (Modern Landing Page) ✅'
            };
            $('#current-theme-display').text(themeNames[themeValue] || 'Unknown');
        }

        function showLoginBlock(condition) {
            if (condition) {
                $('#login_ui_box,#set-homepage-div,#set-homepage-banner-bg').show(100);
            } else {
                $('#login_ui_box,#set-homepage-div,#set-homepage-banner-bg').hide(100);
            }
        }

        function showPrimaryColor(condition) {
            if (condition) {
                $('#primary_color_div').show(100);
            } else {
                $('#primary_color_div').hide(100);
            }
        }
