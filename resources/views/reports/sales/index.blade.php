...
@push('scripts')
    ...
    <script>
        $(function() {
            var dt = window.LaravelDataTables["sales-report-table"];
            ...
            $('#table-actions').append(settingsDropdown);
            var api = dt.api ? dt.api() : dt;
            api.columns().every(function(idx) {
                var headerText = $(api.column(idx).header()).text().trim();
                if (!headerText || headerText === '#' || headerText.toLowerCase() === 'id') {
                    return;
                }
                var checked = api.column(idx).visible();
                var inputId = 'col-toggle-' + idx;
                $('#columnToggleContainer').append(
                    '<div class="custom-control custom-checkbox mb-1">\
                        <input type="checkbox" class="custom-control-input" id="'+inputId+'" data-col="'+idx+'" '+(checked ? 'checked' : '')+'>\
                        <label class="custom-control-label" for="'+inputId+'">'+headerText+'</label>\
                    </div>'
                );
            });
            $('#columnToggleContainer').on('change', 'input[type=checkbox][data-col]', function() {
                var columnIndex = $(this).data('col');
                api.column(columnIndex).visible(this.checked);
            });
        });
    </script>
@endpush
