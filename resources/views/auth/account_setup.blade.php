<x-auth>
    <form id="login-form" action="{{ route('setup_account') }}" class="ajax-form" method="POST">
        ...
        @if (global_setting() && global_setting()->sign_up_terms == 'yes')
            <div class="form-group text-left">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="1" id="terms_and_conditions" name="terms_and_conditions">
                    <label class="form-check-label" for="terms_and_conditions">
                        @lang('app.iAgree')
                        @if(global_setting()->terms_link)
                            <a href="{{ global_setting()->terms_link }}" target="_blank">@lang('app.termsAndConditions')</a>
                        @else
                            @lang('app.termsAndConditions')
                        @endif
                    </label>
                </div>
            </div>
