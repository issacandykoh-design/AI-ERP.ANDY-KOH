    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            <x-forms.select fieldId="locale" :fieldLabel="__('modules.accountSettings.changeLanguage')"
                ...
                @php $selectedMatched = false; @endphp
                @foreach ($languageSettings as $language)
                    @php
                        $code = $language->language_code;
                        $normalizedCode = strtolower($code) === 'tw' ? 'zh-TW' : str_replace('_', '-', $code);
                        $isSelected = ($locale == $normalizedCode);
                        $selectedMatched = $selectedMatched || $isSelected;
                        $flag = ($language->flag_code == 'en') ? 'gb' : $language->flag_code;
                        if ($normalizedCode === 'zh-TW') { $flag = 'tw'; }
                    @endphp
                    <option {{ $isSelected ? 'selected' : '' }}
                            value="{{ $normalizedCode }}">{{ $language->language_name }}</option>
                @endforeach
                @php
                    if (!$selectedMatched) {
                        $curr = language_setting_locale($locale);
                        $currFlag = ($curr && $curr->flag_code) ? $curr->flag_code : 'gb';
                        $currName = ($curr && $curr->language_name) ? $curr->language_name : strtoupper($locale);
                        if (strtolower($locale) === 'en') { $currFlag = 'gb'; }
                        if (strtolower($locale) === 'zh-tw') { $currFlag = 'tw'; $currName = 'Chinese (T)'; }
                @endphp
                        <option selected value="{{ $locale }}">{{ $currName }}</option>
                @php }
                @endphp
            </x-forms.select>
        </div>
            ...
            <div class="col-lg-4">
                ...
                <x-forms.input-group>
                    ...
                    <input type="text" class="form-control height-35 f-14" name="slack_username"
                           id="slack_username" value="{{ $user->employeeDetail ? $user->employeeDetail->slack_username : '' }}">
                </x-forms.input-group>
            </div>
        @endif
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            function updatePhoneCode() {
                var phonecode = $('#country_id').find(':selected').data('phonecode');
                var iso = $('#country_id').find(':selected').data('iso');
