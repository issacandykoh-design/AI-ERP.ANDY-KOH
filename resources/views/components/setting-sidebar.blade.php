...
<div class="settings-sidebar bg-white py-3" id="mob-settings-sidebar">
    ...
    <ul class="settings-menu" id="settingsMenu">
        ...
        @php
            $menuCustomizationInPackage = false;
            if (company() && company()->package && company()->package->module_in_package) {
                $menuCustomizationInPackage = in_array('menu_customization', json_decode(company()->package->module_in_package, true) ?: []);
            }
        @endphp
        @if (user()->is_superadmin || ($menuCustomizationInPackage && (user()->permission('manage_menu_customization') == 'all' || in_array('admin', user_roles()))))
            <x-setting-menu-item :active="$activeMenu" menu="menu_customization" :href="route('menu-customization.index')"
        ...
        @if(iscraveva() || user()->is_superadmin)
            ...
            @if (user()->permission('manage_language_setting') == 'all' || user()->is_superadmin)
