<li {{ $attributes->merge(['class' => 'accordionItem closeIt']) }} @if($menuKey && function_exists('get_custom_menu_order')) data-menu-order="{{ get_custom_menu_order($menuKey) }}" data-menu-key="{{ $menuKey }}" style="order: {{ get_custom_menu_order($menuKey) }};" @endif>

    @if (!is_null($link))
        <a class="nav-item text-lightest f-15 sidebar-text-color" href="{{ $link }}"
           title="{{ $text }}">
            <x-icon :name="$icon" size="18" class="sidebar-icon" />
