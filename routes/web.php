...
// Serve the marketing landing page before login (respects superadmin CMS settings)
Route::get('/', function () {
    if (app()->environment('local')) {
        $candidates = [
            public_path('landing/index.html'),
            public_path('public/landing/index.html'),
            public_path('public/landing/web.html'),
        ];
    ...
    try {
        ...
        if ($global->setup_homepage == 'custom' && $global->custom_homepage_url) {
            return redirect()->away($global->custom_homepage_url);
        }

        if ($global->setup_homepage == 'signup') {
            return redirect()->route('front.signup.index');
        }
        ...
        $controller = app(\App\Http\Controllers\SuperAdmin\FrontendController::class);
...
Route::get('prototype/panelredesign/dashboard', [\App\Http\Controllers\PanelRedesignPublicController::class, 'index'])->name('prototype.panelredesign.dashboard.public');
...
Route::group(['middleware' => ['auth', 'multi-company-select', 'email_verified'], 'prefix' => 'account'], function () {
    ...
    // Invoices
    Route::post('invoices/ocr-scan', [InvoiceController::class, 'ocrScan'])->name('invoices.ocr_scan');
    ...
    Route::group(['prefix' => 'expenses'], function () {
        ...
        Route::post('ocr', [ExpenseController::class, 'ocr'])->name('expenses.ocr');
    });
...
// Test broadcasting
// Emergency menu fix route - accessible when settings menu is hidden
Route::get('/fix-menus', [\App\Http\Controllers\FixMenuController::class, 'resetAllMenus'])->middleware('auth')->name('fix.menus');

Route::get('/test-broadcast', function() {
    try {
        // Use the local NewMessage event and UserChat model
        if (class_exists(\App\Events\NewMessage::class) && class_exists(\App\Models\UserChat::class)) {
            $chat = \App\Models\UserChat::orderByDesc('id')->first();
            if ($chat) {
                broadcast(new \App\Events\NewMessage($chat))->toOthers();
                return response()->json(['success' => true, 'message' => 'NewMessage event broadcasted']);
            }
        }

        return response()->json(['success' => false, 'message' => 'No UserChat found to broadcast']);
    } catch (\Throwable $e) {
...
Route::get('/debug-mail', function() {
    $smtp = \Illuminate\Support\Facades\DB::table('smtp_settings')->first();
    return response()->json([
        'mail_from_address' => config('mail.from.address'),
        'mail_from_name' => config('mail.from.name'),
        'mail_verified' => config('mail.verified'),
        'mail_default' => config('mail.default'),
        'queue_default' => config('queue.default'),
        'smtp_host' => config('mail.mailers.smtp.host'),
        'smtp_port' => config('mail.mailers.smtp.port'),
        'smtp_username' => config('mail.mailers.smtp.username'),
        'db_mail_from_email' => $smtp ? $smtp->mail_from_email : null,
        'db_email_verified' => $smtp ? $smtp->email_verified : null,
        'db_mail_connection' => $smtp ? $smtp->mail_connection : null,
        'db_mail_driver' => $smtp ? $smtp->mail_driver : null,
        'db_mail_host' => $smtp ? $smtp->mail_host : null,
        'db_mail_username' => $smtp ? $smtp->mail_username : null
    ]);
})->name('debug-mail');

Route::get('/send-test-email', function() {
    try {
        \Illuminate\Support\Facades\Notification::route('mail', 'issacandykoh@gmail.com')
            ->notify(new \App\Notifications\TestEmail());
        return response()->json(['success' => true, 'message' => 'Test email dispatched']);
    } catch (\Throwable $e) {
        return response()->json(['success' => false, 'message' => $e->getMessage()]);
    }
})->name('send-test-email');

Route::get('/debug-send-invitation-email', function() {
    try {
        $u = \Illuminate\Support\Facades\DB::table('users')->first();
        $invite = new \App\Models\UserInvitation();
        $invite->user_id = $u ? $u->id : 1;
        $invite->email = 'issacandykoh@gmail.com';
        $invite->invitation_type = 'email';
        $invite->invitation_code = sha1(time() . $invite->user_id);
        \Illuminate\Support\Facades\Notification::send($invite, new \App\Notifications\InvitationEmail($invite));
        return response()->json(['success' => true, 'message' => 'Invitation email dispatched']);
    } catch (\Throwable $e) {
        return response()->json(['success' => false, 'message' => $e->getMessage()]);
    }
})->name('debug-send-invitation-email');

Route::get('/debug-invitations', function() {
    $last = \Illuminate\Support\Facades\DB::table('user_invitations')->orderBy('id','desc')->first();
    return response()->json([
        'count' => \Illuminate\Support\Facades\DB::table('user_invitations')->count(),
        'last' => $last
    ]);
});

Route::get('/debug-create-and-send-invite', function() {
    try {
        $u = \Illuminate\Support\Facades\DB::table('users')->first();
        $invite = new \App\Models\UserInvitation();
        $invite->user_id = $u ? $u->id : 1;
        $invite->email = 'issacandykoh@gmail.com';
        $invite->message = 'Panel route simulation';
        $invite->invitation_type = 'email';
        $invite->invitation_code = sha1(time() . $invite->user_id);
        $invite->status = 'active';
        $invite->save();
        event(new \App\Events\InvitationEmailEvent($invite));
        return response()->json(['success' => true, 'message' => 'Invite created and event dispatched', 'invite_id' => $invite->id]);
    } catch (\Throwable $e) {
        return response()->json(['success' => false, 'message' => $e->getMessage()]);
    }
})->name('debug-create-and-send-invite');

Route::get('/debug-mail-view-test', function() {
    try {
        $n = new class extends \App\Notifications\BaseNotification {
            public function via($notifiable) { return ['mail']; }
            public function toMail($notifiable) {
                $build = parent::build($notifiable);
                return $build->markdown('mail.email', [
                    'content' => 'Demo content with line breaks<br>Second line',
                    'url' => 'https://craveva.com/',
                    'actionText' => 'Open Craveva'
                ]);
            }
        };
        \Illuminate\Support\Facades\Notification::route('mail','issacandykoh@gmail.com')->notify($n);
...
// Prototype Routes - Hardcoded UI/UX Design Mockups
require __DIR__.'/prototype.php';
Route::get('/debug-mail-render-send', function() {
    try {
        $html = \Illuminate\Support\Facades\View::make('mail.email', [
            'content' => 'Render test line 1<br>Render test line 2',
            'url' => 'https://craveva.com/',
            'actionText' => 'Open Craveva'
        ])->render();
        \Illuminate\Support\Facades\Mail::html($html, function ($m) {
            $m->to('issacandykoh@gmail.com')->subject('Render Mail View Test');
        });
        return response()->json(['success' => true]);
    } catch (\Throwable $e) {
        return response()->json(['success' => false, 'message' => $e->getMessage()]);
...
Route::get('/debug-send-password-reset', function() {
    try {
        $email = request('email');
        if (!$email) {
            $email = optional(\Illuminate\Support\Facades\DB::table('user_auths')->first())->email
                ?: optional(\Illuminate\Support\Facades\DB::table('users')->first())->email
                ?: 'issacandykoh@gmail.com';
        }
        $status = \Illuminate\Support\Facades\Password::broker('users')->sendResetLink(['email' => $email]);
        return response()->json(['success' => true, 'status' => $status, 'email' => $email]);
    } catch (\Throwable $e) {
        return response()->json(['success' => false, 'message' => $e->getMessage()]);
    }
});

Route::get('/debug-send-verification-email', function() {
    try {
        $id = request('id');
        $userAuth = $id ? \App\Models\UserAuth::find($id) : \App\Models\UserAuth::first();
        if (!$userAuth) {
            return response()->json(['success' => false, 'message' => 'No user found']);
        }
        $userAuth->sendEmailVerificationNotification();
        return response()->json(['success' => true, 'user_auth_id' => $userAuth->id]);
    } catch (\Throwable $e) {
        return response()->json(['success' => false, 'message' => $e->getMessage()]);
    }
});

Route::get('/debug-dev-login-sa', function () {
    try {
        $email = request('email', 'auto.sa@example.com');
        $userAuth = \App\Models\UserAuth::where('email', $email)->first();
        if (!$userAuth) {
            return response()->json(['success' => false, 'message' => 'UserAuth not found for '.$email], 404);
        }

        \Illuminate\Support\Facades\Auth::login($userAuth, true);

        $user = \App\Models\User::where('user_auth_id', $userAuth->id)->first();
...
Route::get('/debug-telescope-reports-count', function () {
    try {
        $rows = \App\Models\TelescopeReport::orderByDesc('last_seen_at')->take(5)->get();
...
Route::get('/debug-throw', function () {
    throw new \RuntimeException('Debug crash');
});

// React starter page (public)
Route::get('/react/{any?}', function () {
    return view('react.index', [
        'pageTitle' => 'React Starter'
    ]);
})->where('any', '.*');

// moved to routes/SuperAdmin/web.php under auth + super-admin
    Route::get('prototype/panelredesign/dashboard', [\App\Http\Controllers\PanelRedesignPreviewController::class, 'index'])->name('prototype.panelredesign.dashboard');
