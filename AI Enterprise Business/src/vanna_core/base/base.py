...
from ..analyzer import DataAnalyzer, ReportGenerator
...
class VannaBase(ABC):
    def __init__(self, config=None):
        ...
        self._data_analyzer = None  # 延迟初始化，因为需要submit_prompt方法
    ...
    def generate_followup_questions(
        ...
        **Example:**
        ...
        # 创建数据的文字描述而不是markdown表格
        data_description = f"数据概览：\n"
        data_description += f"- 总行数：{len(df)}\n"
        data_description += f"- 列名：{', '.join(df.columns.tolist())}\n"
        data_description += f"- 前25行数据：\n"
        
        # 将前25行数据转换为文字列表格式
        for i, (idx, row) in enumerate(df.head(25).iterrows()):
            if i < 10:  # 只显示前10行以避免过长
                row_data = []
                for col in df.columns:
                    row_data.append(f"{col}: {row[col]}")
                data_description += f"  第{i+1}行 - {', '.join(row_data)}\n"
        
        if len(df) > 10:
            data_description += f"  ... (还有{len(df)-10}行数据)\n"
    ...
    def generate_summary(self, question: str, df: pd.DataFrame, **kwargs) -> str:
        ...
        **Example:**
        ...
        # 创建数据的文字描述而不是markdown表格
        data_description = f"查询结果数据概览：\n"
        data_description += f"- 总行数：{len(df)}\n"
        data_description += f"- 列名：{', '.join(df.columns.tolist())}\n"
        
        # 如果数据行数较少，显示所有数据；否则只显示前10行
        display_rows = min(len(df), 10)
        data_description += f"- 数据内容（前{display_rows}行）：\n"
        
        for i, (idx, row) in enumerate(df.head(display_rows).iterrows()):
            row_data = []
            for col in df.columns:
                row_data.append(f"{col}: {row[col]}")
            data_description += f"  第{i+1}行 - {', '.join(row_data)}\n"
        ...
        if len(df) > display_rows:
            data_description += f"  ... (还有{len(df)-display_rows}行数据)\n"
    ...
    def _get_data_analyzer(self):
        ...
        if self._data_analyzer is None:
            # 创建一个继承DataAnalyzer的类，实现submit_prompt方法
            class VannaDataAnalyzer(DataAnalyzer):
                def __init__(self, vanna_instance, config=None):
                    super().__init__(config)
                    self.vanna = vanna_instance
                
                def submit_prompt(self, prompt: str, **kwargs) -> str:
                    return self.vanna.submit_prompt(prompt, **kwargs)
            
            self._data_analyzer = VannaDataAnalyzer(self, self.config)
    ...
    def analyze_data(
        ...
        **Example:**
        ```python
        analysis = vn.analyze_data("What are the top 10 customers by sales?", sql, df)
        ```

        使用LLM分析SQL查询结果，生成数据洞察和建议。

        Args:
            question (str): 用户的原始问题
            sql (str): 执行的SQL查询
            df (pd.DataFrame): 查询结果数据
            context (str): 额外的上下文信息

        Returns:
            dict: 包含分析结果的字典，包含summary、key_findings、insights、recommendations等
        """
        analyzer = self._get_data_analyzer()
        return analyzer.analyze_data(question, sql, df, context)
    ...
    def generate_report(
        ...
        **Example:**
        ```python
        report = vn.generate_report("What are the top 10 customers by sales?", analysis_result)
        ```
        ...
        Args:
            question (str): 用户的原始问题
            analysis_result (dict): 数据分析结果
            format (str): 报告格式，"text" 或 "json"
        ...
        if format == "json":
            return self.report_generator.generate_json_report(
                question, analysis_result, **kwargs
            )
        else:
            return self.report_generator.generate_report(
                question, analysis_result, include_metadata
            )
    ...
    def ask(
        ...
        analyze_data: bool = True,  # 是否进行数据分析
        visualize: bool = False,  # 可视化作为可选项，默认关闭
        allow_llm_to_see_data: bool = False,
        report_format: str = "text",  # 报告格式：text 或 json
    ) -> Union[
        Tuple[
            Union[str, None],
        ...
        **Example:**
        ...
        Ask Vanna.AI a question and get intelligent data analysis and insights.
        
        新的工作流程：用户提问 → 生成SQL → 执行SQL → 数据分析 → 智能报告
        ...
        Args:
            ...
            analyze_data (bool): Whether to perform intelligent data analysis.
            visualize (bool): Whether to generate plotly visualization (optional).
            allow_llm_to_see_data (bool): Whether to allow the LLM to see the data.
            report_format (str): Report format - "text" or "json".

        Returns:
            Tuple[str, pd.DataFrame, str, plotly.graph_objs.Figure]: SQL query, data, analysis report, and optional figure.
        """
        ...
        if analyze_data and len(df) > 0:
            try:
                if print_results:
                    print(f"\n🤖 **正在进行智能数据分析...**")
                
                # 进行数据分析
                analysis_result = self.analyze_data(question, sql, df)
                
                # 生成报告
                if report_format == "json":
                    analysis_report = self.report_generator.generate_json_report(
                        question=question,
                        analysis_result=analysis_result,
                        sql=sql
                    )
                    import json
                    analysis_report = json.dumps(analysis_report, ensure_ascii=False, indent=2)
                else:
                ...
                if print_results:
                    ...
                    if report_format == "json":
                        import json
                        print(json.dumps(json.loads(analysis_report), ensure_ascii=False, indent=2))
                    else:
                        print(analysis_report)
                        
            except Exception as e:
                print(f"⚠️  数据分析过程中出现错误: {e}")
                analysis_report = f"数据分析失败: {str(e)}"
        ...
        return sql, df, analysis_report, fig
